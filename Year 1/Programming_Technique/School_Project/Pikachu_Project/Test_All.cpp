#include <iostream>
#include <cstring>
#include <fstream> // file background
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <windows.h> // 
...
static bool gameOver = 0; // 0 false 1 true 2 lose
static int score = 0;
...
void drawTitle(){
    SetPosition(0,0);
    fstream fi;
    string s = "Ascii_Art/PokemonTitle.txt";
    string line;
    fi.open(s);
    int count = 0;
    // system("cls");
...
bool moveSuggestion(Points** board, int size){
    // Stretch the matrix into an array 
...
void linkedListMove(Points** pHead, Points** board, int size, int direction){
    switch(getch()){
        ...
        case 13: // ENTER BUTTON
            ...
            if (checkEndGame(board, size))
                gameOver = 1;
            if (checkPlayable(board, size) == false)
                gameOver = 2;
            // else if (checkEndGame(board, size) == false)
            //     gameOver = 2;
            // if (moveSuggestion(board, size) == false){
            //     PlaySoundLose();
            //     gameOver = 2;
            // }
            break;
...
void EasyMode(int size){
    // int size = EASY;
    PlaySoundBackGround0();
    Points** board = newMatrix(size);
    shuffleMatrix(board, size);
    Setup(board, 50*3, 20*3, size);
    Play(board, size);
    getch();
}
...
void SlidingModeDown(int n){
    PlaySoundBackGround1();
...
void SlidingModeRight(int n){
    ...
    shuffleMatrix(board, n);
    Setup(board, 50*3,20*3, n);
    Play_LinkedList(pHead, board, n, R);
    getch();
}

void SlidingModeUp(int n){
    // int n = EASY;
    PlaySoundBackGround2();
    Points** board = newMatrix(n);
    Points** pHead = newLinkedListLU(n, n);
    shuffleMatrix(board, n);
    Setup(board, 50*3,20*3, n);
    Play_LinkedList(pHead, board, n, U);
    getch();
}

void SlidingModeLeft(int n){
    // int n = EASY;
    PlaySoundBackGround2();
    Points** board = newMatrix(n);
    Points** pHead = newLinkedListLU(n, n);
    shuffleMatrix(board, n);
    Setup(board, 50*3,20*3, n);
    Play_LinkedList(pHead, board, n, L);
    getch();
    PlaySound(NULL, NULL, 0);
}

void PreMode(int size){
    PlaySoundBackGround0();
    Points** board = newMatrix(size);
    shuffleMatrix(board, size);
    Setup(board, 50*3, 20*3, size);
...
void Help1()
{
    ...
    std::cout << "r";
    SetPosition(22, 30);
    std::cout << ">> Shuffle (Exclude easy mode)";

    SetPosition(24, 5);
    std::cout << "esc";
    SetPosition(24, 30);
    std::cout << ">> quit level (mean you lose)";
...
void Help2()
{
    ...
    SetPosition(13, 5);
    std::cout << "There are 2 levels, with increasing board sizes.";

    SetColor(BLACK, RED);
    SetPosition(16, 5);
    std::cout << "HARD: Cell shifting after every move, ";
    std::cout << "There are 4 levels.";
...
ONCEAGAIN:
    ...
    cout << "\n1 is easy mode.";
    cout << "\n2 is hard mode.";
    cout << "\n3 is premode(use 'y' to confirm all the selected move)";
    cout << "\n'u' is for manual.";
    cout << "\n'm' is for level infomation.";
    cout << "\nEnter 0 to end game: ";
    SetColor(BLACK, WHITE);
    switch (getch())
    {
    ...
    case '2':
        SetPosition(0, 7*10);
        cout << "Sliding Left";
        SlidingModeLeft(MEDIUM);
        gameOver = 0;

        SetPosition(0, 7*10);
        cout << "Sliding Right";
        SlidingModeRight(MEDIUM);
        gameOver = 0;

        SetPosition(0, 7*10);
        cout << "Sliding Up";
        SlidingModeUp(MEDIUM);
        gameOver = 0;

        SetPosition(0, 7*10);
        cout << "Sliding Down";
        SlidingModeDown(MEDIUM);
        gameOver = 0;
        break;
    case '3':
        PreMode(EASY);
